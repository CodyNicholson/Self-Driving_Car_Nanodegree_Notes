...
When we design a Kalman Filter we need two things: A state transition function for the state (usually a matrix X), and a measurements function for the measurements Z
...
def kalman_filter(x, P):
    for n in range(len(measurements)):
        ...
        Z = matrix([[measurements[n]]]) # Make a measurement matrix of the nth measurement
        y = Z - (H * x) # Error calculation
        S = H * P * H.transpose() + R # The matrix S with a transpose
        K = P * H.transpose() * S.inverse() # The Kalman gain K with the inverse
        x = x + (K * y) # Prediction
